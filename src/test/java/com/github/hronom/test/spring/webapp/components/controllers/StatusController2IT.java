...
public class StatusController2IT {
    ...
    public void testV1() throws Exception {
        mockMvc.perform(get("/v1/status")).andDo(print()).andExpect(status().isOk())
            .andExpect(content().json("{\"status\":true}"));
    }

    @Test
    public void testV2() throws Exception {
        mockMvc.perform(get("/v2/status")).andDo(print()).andExpect(status().isOk())
            .andExpect(content().json("{\"status\":true}"));
    }
}
